...
In order to compare the relative contributions of $GEZI$ and $S_I$ to net glucose uptake, we determined the proportion of glucose uptake due to glucose itself and due to insulin, in both subjects with NGT and those with T2D. At the steady state, the fraction of glucose disposal mediated by $GEZI$ can be calculated as: $\frac{GEZI}{GEZI+S_I*I_{basal}}$, using the individual estimates of $GEZI$ and $S_I$ and measured $I_{basal}$ of each subject. In 343 NGT subjects, $GEZI$ on average accounts for 86.3\% of the total net glucose disposal with a standard deviation of 10.3\%, which suggests a major contribution of glucose effectiveness to the total net glucose disposal. In 154 T2D subjects, the mean portion mediated by $GEZI$ is 85.7\% with a standard deviation of 16.4\%. Also, we simulated IVGTT experiments of 268 subjects and calculated the accumulative glucose diposal mediated by $GEZI$ and $S_I$ till the end of studies. In 238 NGT subjects, the fraction of glucose disposition by $GEZI$ has a mean value of 73.0\% with a standard deviation of 12.3\%, which is basically consistent with the reported range (45\% to 65\%) in \citet{Alford_2018}. In 30 T2D patients, 49.3\% of net glucose disposition after an IV load can be attributed to $GEZI$ on average with a standard deviation of 17.5\%. These model-based results further quantify the important role of glucose effectiveness in the regulation of glucose homeostasis \citep{Dube2015}.
...
% agbaje_2003 only focused on base model
% discuss and compare with the results in Cobelli paper
% compare to the reulsts in Theodorakis2017
% The incorporation of covariates into the model reduces the inter-individual variability of $GEZI$, $S_I$ and $V$ from 50.9\%,  113\% and 34.4\% to 46.5\%, 88.6\% and 26.8\%, respectively (Table. \ref{tab:prm estimates}).
% The model of V
% previous studies
% V covariate model
